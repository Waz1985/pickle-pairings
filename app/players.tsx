...
export default function PlayersScreen() {
  // ✅ ahora también leemos courts
  const params = useLocalSearchParams<{ mode?: Mode; count?: string; courts?: string }>();

  const mode = (params.mode ?? "random") as Mode;
  const count = Number(params.count ?? "8");
  const courtsNumber = Math.max(1, Math.floor(Number(params.courts ?? "1"))); // ✅ courtsNumber existe aquí
  ...
  function handleContinue() {
    ...
    // ✅ ahora sí compila
    startSession(mode, players, courtsNumber);
  ...
  return (
    <View style={{ flex: 1 }}>
      <ScrollView contentContainerStyle={{ padding: 16, gap: 12 }}>
        ...
        <Text style={{ opacity: 0.7 }}>Canchas: {courtsNumber}</Text>
